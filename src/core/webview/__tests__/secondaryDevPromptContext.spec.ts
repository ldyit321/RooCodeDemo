...
describe("secondaryDevPromptContext", () => {
	...
	it("injects secondary dev settings for huayun mode without exposing the raw client secret", () => {
		...
		expect(result).toContain("Frontend implementation must use Vue in HUAYUN Secondary Dev mode")
		...
		expect(result).toContain("Cookie + Header dual-channel strategy")
		expect(result).toContain("`X-Huayun-Session`")
		expect(result).toContain("`credentials: include`")
		expect(result).toContain("cookie channel first and fall back to the configured session header")
		expect(result).toContain("credentialed iframe use case")
