...
export const buildSecondaryDevPromptContext = (state: SecondaryDevPromptState): string => {
	...
	return [
		...
		"- When the user asks whether an API, contract, field, enum, or capability exists, inspect HUAYUN rule materials first, especially `.roo/rules-huayun-secondary-dev`, `.roo/skills`, and `exports/skills`, before concluding from business implementation code.",
		"- When searching HUAYUN rules, inspect the current workspace first, especially `.roomodes`, `.roo/rules-huayun-secondary-dev`, and `.roo/skills` under the active project root.",
		...
		"- Use user-home or global rule directories only as a fallback when the current workspace does not contain the relevant HUAYUN rule or skill.",
		"- Treat internal rule paths such as `.roo/...` and `.roomodes` as implementation details, not user-facing concepts, unless the user explicitly asks for those paths.",
