...
async function getModeSpecificRooDirectories(cwd: string, mode: string, enableSubfolderRules: boolean): Promise<string[]> {
...
export async function addCustomInstructions(
	...
	if (mode) {
		...
		if (modeRules.length > 0) {
			modeRuleContent = "\n" + modeRules.join("\n\n")
			usedRuleFile = mode === BUILT_IN_HUAYUN_MODE_SLUG ? `built-in and workspace rules-${mode} directories` : `rules-${mode} directories`
		} else {
